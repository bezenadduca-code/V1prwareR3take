...
-- Guest Auto Configuration (moved from Guest Auto Tab)
local guestAutoCfg = {
    hitboxEnabled    = false,
    animEnabled      = false,
...
-- ============================================================
-- AUTO BLOCK SECTION (From Guest Auto Tab - Now in Core)
-- ============================================================
local secAutoBlock = tabGuestCore:Section({ Title = "Auto Block (Hitbox & Animation)", Opened = true })
...
secAutoBlock:Button({ 
    Title = "Refresh Auto-Block Connections", 
    Callback = function()
...
local function atFireAbility()
    -- Auto fire Veeronica trick ability
    local rem = hbGetRemote()
    if not rem then return end
    
    local buf = buffer.fromstring("\x03\x01\x00\x00\x00")
    pcall(function() rem:FireServer("UseActorAbility", {[1] = buf}) end)
end
