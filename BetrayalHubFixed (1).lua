-- BetrayalHub - Complete Final Version
...
local function toggleJump(enabled)
    pcall(function()
        ...
        -- Also disable jump power on humanoid
        ...
        if enabled then
            ...
            -- Set jump power to 0 to prevent keyboard jumping too
            if hum then
                hum.JumpPower = 0
                hum.Jump = false
            end
...
lp.CharacterAdded:Connect(function(character)
    task.delay(0.5, function()
        pcall(function()
            ...
            -- Also reapply jump disable on new character
...
-- Also connect to Humanoid on existing character
task.delay(0.5, function()
    pcall(function()
        if jumpDisabled then
            local char = lp.Character
...
local aim = {
    enabled    = false,
    slashUnlock = true,  -- if true: slash releases lock; if false: only death releases lock
    maxDist    = 80,
    locked     = false,
    target     = nil,
    lockConn   = nil,
...
local function aimDoCamera(headPos)
    ...
    local lerpSpd  = dist > 40 and 0.9 or 0.5
    local dir      = (hrp.Position - headPos)
    local camPos   = hrp.Position + (dist > 0.01 and dir.Unit * 5 or Vector3.zero) + Vector3.new(0, 3, 0)
    cam.CFrame     = cam.CFrame:Lerp(CFrame.new(camPos, headPos), lerpSpd)

    pcall(function()
        local ld = Vector3.new(headPos.X - hrp.Position.X, 0, headPos.Z - hrp.Position.Z)
        if ld.Magnitude > 0.01 then
            hrp.CFrame = hrp.CFrame:Lerp(CFrame.new(hrp.Position, hrp.Position + ld.Unit), 0.6)
        end
    end)
    ...
    if mousemoverel then
        pcall(function()
            ...
            if onScreen then
                ...
                mousemoverel((sp.X - vs.X/2)*0.6, (sp.Y - vs.Y/2)*0.6)
            end
        end)
    end
end
...
local function aimLock(targetHRP)
    ...
    aim.lockConn = svc.Run.RenderStepped:Connect(function()
        ...
        if not hum or hum.Health <= 0 then
            -- survivor died → always unlock regardless of slashUnlock setting
            aimUnlock()
            return
        end
...
-- ---- HOOK via __namecall (intercepts outgoing FireServer) ----
...
aimOldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if method == "FireServer" and args[1] == "UseActorAbility" then
        if aim.enabled then
            task.spawn(function()
                pcall(function()
                    -- Only act if we are the killer
                    if not aimIsKiller() then return end

                    local rawData = args[2] and args[2][1]
                    local name = rawData and hbReadName(rawData) or ""
                    aim.lastAbility = name

                    -- Slash toggle logic
                    if name:lower():find("slash") then
                        if aim.slashUnlock then
                            -- ON → slash releases lock
                            aimUnlock()
                        end
                        -- OFF → do nothing, stay locked until survivor dies
                        return
                    end

                    -- Any other ability → lock nearest survivor
...
-- ============================================================
-- GROUNDBULB & VINE ESP (PURPLE - OVERRIDES GAME HIGHLIGHTS - WORKING)
-- ============================================================
...
local gvConfig = {
    ShowGroundbulb = true,
    ShowVine = true,
    HighlightColor = Color3.fromRGB(170, 0, 255), -- PURPLE
...
-- Replace the toggle in Categories section
catSection:Toggle({ 
...
print("[GV ESP] Loaded - Purple highlights, text lowered")
...
-- ============================================================
-- GUEST 1337 - CORE BLOCK TAB (FIXED - Auto-restores vision on new round)
-- ============================================================
...
-- ============================================================
-- FIX: Vision cleanup and restoration on round start
-- ============================================================
...
-- Clean up round watcher connections
...
local function g_setupRoundWatcher()
    ...
    -- Also watch workspace for new Map
...
-- VISION CONE - Auto-restores on new round
...
local function g_updateVision()
    ...
    -- Check if parts were destroyed (round reset) and recreate them
    ...
    local root = g_getRoot()
    if not root then 
        -- If we have parts but no root, keep them but don't update position
        -- They'll update when root exists
        return 
    end
    ...
    -- Recreate any missing parts (auto-restores on new round)
        ...
        if animator then
            g.aimConnection = animator.AnimationPlayed:Connect(function(track)
                ...
                if g_trackedPunchAnims[animId] then
                    -- Only save once per punch so we don't overwrite the original
...
-- MAIN RENDER LOOP - Updates vision every frame (auto-restores on new round)
svc.Run.RenderStepped:Connect(function()
    -- Aim punch
    if not guestCfg.aimPunchActive then
        -- Feature was toggled off mid-punch — restore immediately
    ...
    elseif g.humanoid and g.hrp and g.punchAiming then
        ...
        if elapsed > guestCfg.aimPunchDuration or autoPunchFired then
            ...
            -- Save WS/JP mid-loop (AutoRotate already saved at trigger time)
    ...
    -- Update vision (auto-restores if parts were destroyed)
    pcall(g_updateVision)
    pcall(g_updatePing)
...
lp.CharacterAdded:Connect(function(c) 
    task.delay(0.5, function() 
        ...
        -- Recreate vision on character spawn
        task.wait(0.1)
        pcall(g_updateVision)
    end) 
end)
...
secCore:Toggle({ Title="Show Vision Cone", Type="Checkbox", Flag="guestShowVisionCone", Default=guestCfg.showVisionCone,
    Callback=function(v) 
        ...
        if not v then 
            ...
            -- Immediately show vision if enabled
            task.wait(0.1)
            pcall(g_updateVision)
        end
    end })
...
print("[Guest Core] Loaded with auto-restore on new rounds")

end -- tabGuestCore

-- ============================================================
-- GUEST 1337 - HDT TAB (Activates on Block Remote with Cooldown)
-- ============================================================
...
local guestCfg = {
    ...
    hdtShowRange = false,
    hdtFaceKiller = true,
    hdtRespectCooldown = true, -- Respect the 27s cooldown
}
...
local hdt = {
    ...
    remote = nil,
    lastBlockTime = 0,
    blockCooldownDuration = 27, -- 27 seconds cooldown
}
...
-- Check if block is ready (no cooldown)
local function hdt_isBlockReady()
    -- Check UI cooldown display first (most accurate)
    local ui = lp:FindFirstChild("PlayerGui")
    if ui then
        local m = ui:FindFirstChild("MainUI")
        if m then
            ...
            if ac then
                ...
                if bl then
                    ...
                    if ct and ct:IsA("TextLabel") then
                        ...
                        return true -- No cooldown showing
                    end
                end
            end
        end
    ...
    -- Fallback: check time since last block
    local timeSinceBlock = tick() - hdt.lastBlockTime
    return timeSinceBlock >= hdt.blockCooldownDuration
end
...
local function hdt_execute(killer, killerHRP)
    ...
    -- Check if block is ready (respect cooldown)
    if guestCfg.hdtRespectCooldown and not hdt_isBlockReady() then
        print("[HDT] ⏳ Block on cooldown, HDT skipped")
        return
    end
...
-- DETECT BLOCK REMOTE - This is where HDT activates
local function hdt_setupRemoteDetection()
    local remote = hdt_getRemote()
    if not remote then
        warn("[HDT] Remote not found - feature disabled")
        return
    end
    ...
    remote.OnClientEvent:Connect(function(action, data)
        ...
        -- Check if it's a block ability
        local name = ""
        pcall(function() name = buffer.tostring(data[1]) end)
        if not name or not name:find("Block") then return end
        
        -- Block was fired! Now check cooldown
        if guestCfg.hdtRespectCooldown and not hdt_isBlockReady() then
            print("[HDT] ⏳ Block on cooldown (27s), HDT skipped")
            return
        end
        
        -- Block is ready, find target and execute HDT
        local killer, killerHRP = hdt_findTarget()
        if killer and killerHRP then
            print("[HDT] 🛡️ Block detected! Executing HDT")
...
-- Also detect from AntiFlickZone (legacy support)
...
secHDT:Button({
    ...
    Callback = function()
        ...
        if guestCfg.hdtRespectCooldown and not hdt_isBlockReady() then
            ui:Notify({
                Title = "Block on Cooldown",
                Content = "Wait " .. math.ceil(hdt.blockCooldownDuration - (tick() - hdt.lastBlockTime)) .. "s",
                Duration = 2
            })
            return
        end
...
print("[Guest HDT] ✅ Loaded - Activates on block remote with 27s cooldown")
    ...
    remote.OnClientEvent:Connect(function(action, data)
        ...
        for _, killer in ipairs(kf:GetChildren()) do
            ...
            if hrp then
                ...
                if d < bd2 and d <= 20 then -- Use default detection range
                    bd2 = d; best = killer
                end
            end
        end
