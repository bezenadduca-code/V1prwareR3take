...
-- ============================================================
-- FIXED GENERATOR TRACKER (4 Puzzles, 25% each)
-- ============================================================
...
local function genGetLabel(done, total)
    local pct = total > 0 and math.floor((done / total) * 100) or 0
    return string.format("Generator  %d/%d  (%d%%)", done, total, pct)
end

local function genUpdateAllLabels()
    pcall(function()
        local txt = genGetLabel(genTracker.done, genTracker.total)
        for obj, lbl in pairs(genTracker.labels) do
...
local function genIsComplete(obj)
    ...
    -- Method 1: Check for "Completed" attribute
    if obj:GetAttribute("Completed") == true then return true end
    
    -- Method 2: Check for "Progress" attribute at 100
    local progress = obj:GetAttribute("Progress")
    if progress and tonumber(progress) >= 100 then return true end
    
    -- Method 3: Check for "Powered" attribute
    if obj:GetAttribute("Powered") == true then return true end
    
    -- Method 4: Check for "Done" attribute
    if obj:GetAttribute("Done") == true then return true end
    
    -- Method 5: Check all children for completion indicators
    for _, child in ipairs(obj:GetChildren()) do
        if child:IsA("BoolValue") then
            if child.Name == "Completed" and child.Value == true then return true end
            if child.Name == "Powered" and child.Value == true then return true end
            if child.Name == "Done" and child.Value == true then return true end
            if child.Value == true then return true end
        end
        if child:IsA("IntValue") and child.Name == "Progress" and child.Value >= 100 then
            return true
        end
        if child:IsA("NumberValue") and child.Name == "Progress" and child.Value >= 100 then
            return true
        end
    end
    
    -- Method 6: Check ALL descendants
    for _, d in ipairs(obj:GetDescendants()) do
        if d:IsA("BoolValue") and d.Value == true then
            if d.Name == "Completed" or d.Name == "Powered" or d.Name == "Done" then
                return true
            end
        end
        if d:IsA("IntValue") and d.Name:match("Progress") and d.Value >= 100 then
            return true
        end
        if d:IsA("NumberValue") and d.Name:match("Progress") and d.Value >= 100 then
            return true
        end
    end
...
local function genRecount()
    if not esp.enabled then return end
    
    local nowDone = 0
    local total = 0
    
    for o in pairs(genTracker.labels) do
        if o and o.Parent then
            total = total + 1
            if genIsComplete(o) then 
                nowDone = nowDone + 1 
            end
        end
    end
    
    if genTracker.done ~= nowDone or genTracker.total ~= total then
        genTracker.done = nowDone
        genTracker.total = total
        genUpdateAllLabels()
    end
end

local function genWatchObj(obj)
    pcall(function()
        if genTracker.attrConns[obj] then return end
        
        local conns = {}
        
        -- Watch attribute changes
        table.insert(conns, obj.AttributeChanged:Connect(function(attr)
            if attr == "Completed" or attr == "Progress" or attr == "Powered" or attr == "Done" then
                genRecount()
            end
        end))
        
        -- Watch children for value changes
        local function watchChildren(parent)
            for _, child in ipairs(parent:GetChildren()) do
                if child:IsA("BoolValue") or child:IsA("IntValue") or child:IsA("NumberValue") then
                    table.insert(conns, child:GetPropertyChangedSignal("Value"):Connect(genRecount))
                end
        ...
        table.insert(conns, obj.DescendantAdded:Connect(function(d)
            task.wait(0.1)
            if d:IsA("BoolValue") or d:IsA("IntValue") or d:IsA("NumberValue") then
                table.insert(conns, d:GetPropertyChangedSignal("Value"):Connect(genRecount))
                genRecount()
            end
        end))
        
        genTracker.attrConns[obj] = { 
            Disconnect = function()
...
local function genClearTracker()
    pcall(function()
        for _, conn in pairs(genTracker.attrConns) do 
            pcall(function() conn:Disconnect() end) 
        end
        genTracker.attrConns = {}
        genTracker.labels    = {}
        genTracker.total     = 0
        genTracker.done      = 0
    end)
    ...
    pcall(function()
        ...
        if labelText == "Generator" or (not isCharacter and obj.Name == "Generator") then
            ...
            for o in pairs(genTracker.labels) do
                if o and o.Parent then
                    total = total + 1
                    if genIsComplete(o) then done = done + 1 end
                end
            end
            genTracker.total = total
            genTracker.done = done
            genUpdateAllLabels()
            genWatchObj(obj)
        end
        ...
        table.insert(esp.connections, obj.AncestryChanged:Connect(function()
            if not obj.Parent then
                pcall(function()
                    ...
                    if genTracker.labels[obj] then
                        genTracker.labels[obj] = nil
                        genTracker.attrConns[obj] = nil
                        ...
                        genTracker.total = total; genTracker.done = done
                        genUpdateAllLabels()
                    end
            ...
            if mc then
                ...
                for _, o in ipairs(mc:GetChildren()) do
                    if o.Name == "Generator" and o.Parent then 
                        table.insert(genObjs, o) 
                    end
                end
                genTracker.total = #genObjs
                for _, o in ipairs(genObjs) do 
                    addESP(o, "Object", COLORS.Generator, "Generator") 
                end
                local done = 0
                for o in pairs(genTracker.labels) do
                    if o and o.Parent and genIsComplete(o) then done = done + 1 end
                end
                genTracker.done = done
                genUpdateAllLabels()
            end
