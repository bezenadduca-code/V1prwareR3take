...
local function genIsComplete(obj)
    ...
    for _, d in ipairs(obj:GetDescendants()) do
        if d:IsA("BoolValue") and d.Value == true then return true end
    end
    return false
end
...
local function genRecount()
    ...
    for o in pairs(genTracker.labels) do
        if o and o.Parent and genIsComplete(o) then nowDone = nowDone + 1 end
    end
...
local function genWatchObj(obj)
    pcall(function()
        ...
        local conns = {}
        table.insert(conns, obj.AttributeChanged:Connect(genRecount))
        for _, d in ipairs(obj:GetDescendants()) do
        ...
        genTracker.attrConns[obj] = { Disconnect = function()
...
local function genClearTracker()
    pcall(function()
        for _, conn in pairs(genTracker.attrConns) do pcall(function() conn:Disconnect() end) end
...
local function espCreate(obj, color, labelText, isCharacter)
    pcall(function()
        ...
        if labelText == "Generator" or (not isCharacter and obj.Name == "Generator") then
            ...
            genTracker.total = 0
            local count = 0
            for o in pairs(genTracker.labels) do if o and o.Parent then count = count + 1 end end
            genTracker.total = count
            local done = 0
            for o in pairs(genTracker.labels) do
        ...
        if esp.showGenerators then
            ...
            if mc then
                ...
                for _, o in ipairs(mc:GetChildren()) do
                    if o.Name == "Generator" and o.Parent then table.insert(genObjs, o) end
                end
                genTracker.total = #genObjs
                for _, o in ipairs(genObjs) do addESP(o,"Object",COLORS.Generator,"Generator") end
                local done = 0
                for o in pairs(genTracker.labels) do
                    if o and o.Parent and genIsComplete(o) then done = done + 1 end
                end
