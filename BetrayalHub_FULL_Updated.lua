...
local gvConfig = {
    HighlightColor = Color3.fromRGB(170, 0, 255), -- Purple color
...
local function gvHighlightEntity(model, displayName)
    ...
    if gvTrackedEntities[model] then return end -- Already tracked
    ...
    if not adornPart then
        warn("[GV-ESP] No part found for:", model.Name)
        return
    end
    ...
    highlight.Name = "GV_EntityHighlight"
    ...
    -- Create a BillboardGui to display the text above the top of the entity
    local billboard = Instance.new("BillboardGui")
    billboard.Name = "GV_EntityTag"
    ...
    -- Store the entity and its associated objects for cleanup
    gvTrackedEntities[model] = {
        Model = model,
        Highlight = highlight,
        Billboard = billboard,
    }

    print(string.format("[GV-ESP] Highlighted: %s as %s", model.Name, displayName))
end
...
local function gvEvaluateAndHighlight(child)
    ...
    -- Deep checks if name doesn't match directly
    ...
    if isGroundBulb and gvEsp.showGroundbulb and not gvTrackedEntities[child] then
        gvHighlightEntity(child, "GroundBulb")
    elseif isVine and gvEsp.showVine and not gvTrackedEntities[child] then
        gvHighlightEntity(child, "Vine")
    end
end

-- Function to scan the workspace for new Vines and GroundBulbs
local function gvScanForSpawns()
...
local function gvCleanupEntities()
    for model, data in pairs(gvTrackedEntities) do
        if not model or not model.Parent or not model:IsDescendantOf(svc.WS) then
...
local gvConnection = nil
local gvListenerConnection = nil

local function gvStart()
    if gvEsp.on then return end
    gvEsp.on = true
    
    print("[GV-ESP] Starting...")
    task.wait(0.5)
    gvScanForSpawns()
    
    if gvConnection then gvConnection:Disconnect() end
    gvConnection = svc.Run.Heartbeat:Connect(function()
        gvUpdateLoop()
    end)
    
    -- Listen for new models added to the "Ingame" folder
    local function gvSetupListener()
        local map = svc.WS:FindFirstChild("Map")
        if map then
            local ingame = map:FindFirstChild("Ingame")
            if ingame then
                if gvListenerConnection then gvListenerConnection:Disconnect() end
                gvListenerConnection = ingame.ChildAdded:Connect(function(child)
...
local function gvStop()
    ...
    if gvConnection then 
        gvConnection:Disconnect()
        gvConnection = nil
    end
    
    if gvListenerConnection then
        gvListenerConnection:Disconnect()
        gvListenerConnection = nil
    end
...
secGVEsp:Slider({
    ...
    Callback = function(v)
        pcall(function()
            ...
            for _, data in pairs(gvTrackedEntities) do
                if data.Highlight then
...
secGVEsp:Button({
    ...
    Callback = function()
        pcall(function()
            ...
            gvTrackedEntities = {}
            print("[GV-ESP] Cleared all highlights")
        end)
    end
})
...
print("[GV-ESP] Groundbulb & Vine ESP integrated")
