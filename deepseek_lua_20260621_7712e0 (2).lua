...
-- ============================================================
-- FIXED GENERATOR TRACKER (Live Progress at 25%, 50%, 75%, 100%)
-- ============================================================
...
local function genIsComplete(obj)
    ...
    local prog = genGetProgress(obj)
    if prog >= 100 then return true end
...
local function genRecount()
    ...
    if genTracker.done ~= nowDone or genTracker.total ~= total then
        ...
        print(string.format("[Generator] %d/%d complete (%d%%)", nowDone, total, total > 0 and math.floor((nowDone/total)*100) or 0))
    end
end

-- WATCH GENERATOR FOR PROGRESS CHANGES
local function genWatchObj(obj)
    pcall(function()
        ...
        table.insert(conns, obj.AttributeChanged:Connect(function(attr)
            if attr == "Progress" or attr == "Completed" or attr == "Powered" or attr == "Done" then
                local prog = genGetProgress(obj)
                genTracker.progress[obj] = prog
                print(string.format("[Generator] Progress: %d%%", prog))
                genRecount()
            end
        end))
        
        -- Watch children for Progress changes
        local function watchChildren(parent)
            for _, child in ipairs(parent:GetChildren()) do
                if child:IsA("IntValue") and child.Name == "Progress" then
                    table.insert(conns, child:GetPropertyChangedSignal("Value"):Connect(function()
                        local prog = child.Value
                        genTracker.progress[obj] = prog
                        print(string.format("[Generator] Progress: %d%%", prog))
                        genRecount()
                    end))
        ...
        local initialProg = genGetProgress(obj)
        genTracker.progress[obj] = initialProg
        if initialProg > 0 then
            print(string.format("[Generator] Initial Progress: %d%%", initialProg))
        end
    end)
end
